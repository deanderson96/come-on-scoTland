...
function init() {
  ...
  $('#todayDate').textContent = new Intl.DateTimeFormat('en-GB', { timeZone: APP_CONFIG.displayTimeZone || 'Europe/London', dateStyle: 'full' }).format(new Date());
  loadTournamentData();
}
...
function renderStats() {
  ...
  $('#nextScotlandMatch').innerHTML = next
    ? `<b>${matchLine(next)}</b><br><span>${formatDateTime(next.date, next.hasReliableKickoff)} • ${safe(next.venue, 'Venue TBC')}</span>`
...
function renderFixtures() {
  ...
  $('#fixtureRows').innerHTML = rows.length
    ? rows.map(e => `<tr class="${isScotland(e) ? 'row-scotland' : ''}"><td>${formatDateTime(e.date, e.hasReliableKickoff)}</td><td><span class="badge">${escapeHtml(e.round)}</span></td><td><b>${matchLine(e)}</b></td><td>${escapeHtml(safe(e.venue, 'Venue TBC'))}</td><td>${statusBadge(e)}</td></tr>`).join('')
...
function renderBracket() {
  ...
  $('#bracket').innerHTML = rounds.length
    ? rounds.map(round => `<div class="round"><h3>${escapeHtml(round)}</h3>${knockout.filter(e => e.round === round).map(e => `<div class="tie ${isScotland(e) ? 'scotland-tie' : ''}"><b>${matchLine(e)}</b><p class="meta">${formatDateTime(e.date, e.hasReliableKickoff)}<br>${escapeHtml(safe(e.venue, 'Venue TBC'))}<br>${plainStatus(e)}</p></div>`).join('')}</div>`).join('')
...
function matchCard(event, scotland = false) {
  return `<article class="match-card ${scotland || isScotland(event) ? 'scotland' : ''}"><span class="badge">${escapeHtml(event.round)}</span><div class="teams">${matchLine(event)}</div><p class="meta">${formatDateTime(event.date, event.hasReliableKickoff)}<br>${escapeHtml(safe(event.venue, 'Venue TBC'))}<br>${plainStatus(event)}</p></article>`;
}
...
function normaliseEvent(event) {
  ...
  const rawHomeScore = toNumber(event.intHomeScore);
  const rawAwayScore = toNumber(event.intAwayScore);
  const dateInfo = normaliseDate(event);
  const finished = isFinishedStatus(rawStatus) || hasActuallyFinished(dateInfo.iso, rawStatus, rawHomeScore, rawAwayScore);

  return {
    id: event.idEvent,
    home,
    away,
    homeScore: finished ? rawHomeScore : undefined,
    awayScore: finished ? rawAwayScore : undefined,
    date: dateInfo.iso,
    hasReliableKickoff: dateInfo.hasReliableKickoff,
...
function isCompleted(e) {
  return Boolean(e.isResult) && Number.isFinite(e.homeScore) && Number.isFinite(e.awayScore);
}
...
function plainStatus(e) {
  if (isCompleted(e)) return 'Result';
  if (!e.hasReliableKickoff) return 'Scheduled • Kick-off TBC';
  return 'Scheduled';
}
...
function formatDateTime(value, hasReliableKickoff = true) {
  if (!value) return 'Date TBC';
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return 'Date TBC';
  const dateText = new Intl.DateTimeFormat('en-GB', { timeZone: APP_CONFIG.displayTimeZone || 'Europe/London', dateStyle: 'medium' }).format(date);
  if (!hasReliableKickoff) return `${dateText} • Kick-off TBC`;
  return new Intl.DateTimeFormat('en-GB', { timeZone: APP_CONFIG.displayTimeZone || 'Europe/London', dateStyle: 'medium', timeStyle: 'short', timeZoneName: 'short' }).format(date);
}

function normaliseDate(event) {
  if (event.strTimestamp) {
    const parsed = new Date(event.strTimestamp);
    if (!Number.isNaN(parsed.getTime())) return { iso: parsed.toISOString(), hasReliableKickoff: true };
  }

  const date = event.dateEvent || event.strDate;
  const rawTime = String(event.strTime || '').trim();
  if (!date) return { iso: '', hasReliableKickoff: false };

  const placeholderTime = !rawTime || /^00:00(?::00)?(?:\+00:00)?$/.test(rawTime) || /^TBC$/i.test(rawTime);
  if (placeholderTime) return { iso: `${date}T12:00:00Z`, hasReliableKickoff: false };

  const cleanTime = rawTime.replace(/\+.*$/, '').replace(/Z$/, '');
  const parsed = new Date(`${date}T${cleanTime}Z`);
  if (Number.isNaN(parsed.getTime())) return { iso: `${date}T12:00:00Z`, hasReliableKickoff: false };
  return { iso: parsed.toISOString(), hasReliableKickoff: true };
}

function isFinishedStatus(status) {
  return /match finished|finished|full.?time|^ft$|after extra time|aet|penalties|final result|ended|complete/i.test(String(status || ''));
}

function isScheduledStatus(status) {
  return /not started|scheduled|fixture|preview|postponed|cancelled|tbc/i.test(String(status || ''));
}

function hasActuallyFinished(dateValue, status, homeScore, awayScore) {
  if (!Number.isFinite(homeScore) || !Number.isFinite(awayScore)) return false;
  if (isScheduledStatus(status)) return false;
  const date = new Date(dateValue);
  if (Number.isNaN(date.getTime())) return false;
  const fourHoursAfterKickoff = date.getTime() + 4 * 60 * 60 * 1000;
  return Date.now() > fourHoursAfterKickoff;
}
